...
class SteelCoreDashboard(tk.Tk):
    ...
    def load_settings(self):
        # Selbstheilung: Settings.json prüfen und ggf. reparieren
            ...
            try:
                ...
                if "modules" in s:
                    if isinstance(s["modules"][0], dict):
                        ...
                        # alte Struktur (nur True/False), umwandeln
                        new_mods = []
    ...
    def create_main(self):
        ...
        self.log_box.pack()
        self.footer_ref = None  # Wird im Footer gesetzt
    ...
    def on_drag_motion(self, event):
        # Simuliere Drag mit grid()-Layout
        pass # Optional: später Drag&Drop implementieren
...
# Settings-Datei fallback/aktualisieren
if [ ! -f "$SETTINGS" ]; then
    cat <<JSON > "$SETTINGS"
{
  "theme": "light",
  "sidebar": true,
  "modules": [
    {"active": true, "name": "Modul 1", "icon": ""},
    {"active": true, "name": "Modul 2", "icon": ""},
    {"active": true, "name": "Modul 3", "icon": ""},
    {"active": true, "name": "Modul 4", "icon": ""},
    {"active": true, "name": "Modul 5", "icon": ""},
    {"active": true, "name": "Modul 6", "icon": ""},
    {"active": true, "name": "Modul 7", "icon": ""},
    {"active": true, "name": "Modul 8", "icon": ""},
    {"active": true, "name": "Modul 9", "icon": ""},
    {"active": true, "name": "Modul 10", "icon": ""},
    {"active": true, "name": "Modul 11", "icon": ""},
    {"active": true, "name": "Modul 12", "icon": ""}
  ],
  "font_size": 12,
  "n_cols": 4,
  "n_rows": 3
...
echo -e "ID: 26\\nZeit: $STAMP\\nBeschreibung: Responsive GUI, Update-Panel Bugfix, Barrierefreiheit, modulare Settings\\nDateien:\\n  - $GUIFILE\\n  - $SETTINGS" > creatoros/updater/meta/changes/change_26.txt
echo "Update 26 applied $STAMP" >> creatoros/updater/info-stand.txt
md5sum "$GUIFILE" "$SETTINGS" >> creatoros/updater/CHECKSUMS.txt

echo "✅ Update 26 erfolgreich abgeschlossen."
